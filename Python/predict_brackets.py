...
# Slot height (px) used by format_bracket_html for vertical team positioning.
...
def format_bracket_html(
    ...
    df_r1 = load_bracket_round(data_root, year, 1)
    r1_games = []
    ...
    def gc(rnd, idx):
        """Safe correct accessor."""
        return _get_c(correct_by_round, rnd, idx, is_current)

    # Vertical-position helpers ------------------------------------------------
    # Within a region (REGION_SLOTS tall), round r (1-indexed as R2=1,R3=2,R4=3),
    # item k:  top = ((2^(r+1))*k + 2^r - 1) * _SH
    def region_top(r, k):
