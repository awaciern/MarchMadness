...
X = df_games[feat_list]
y = df_games['Win__1']

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)

# MODEL = 'Logistic_Newton'
# model = LogisticRegression(random_state=0, solver='newton-cg', max_iter=1000).fit(X_train, y_train)
...
# model = LogisticRegression(random_state=0, solver='liblinear', max_iter=1000).fit(X_train, y_train)
# MODEL = 'KNN3'
...
for year in range(2012, 2026):
    ...
    if not os.path.exists('../Predictions2/' + MODEL):
        os.mkdir('../Predictions2/' + MODEL)

    print(pred_file_str)
    with open('../Predictions2/' + MODEL + '/' + str(year) + '.csv', 'w') as pred_file:
...
print(summary_str)
with open('../Predictions2/' + MODEL + '/summary.txt', 'w') as file:
